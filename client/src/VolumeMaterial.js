    ...
    super({
      ...
      fragmentShader: /* glsl */ `
        ...
        // distance to box bounds
          ...
          // To Do: performance drops a lot from here?
          vec4 sceneColor = texture2D(sceneTex, vUv);
          ...
          if ( intersectsBox ) {
            ...
            for ( int i = 0; i < MAX_STEPS; i ++ ) {
              ...
              // get the distance to surface and exit the loop if we're close to the surface
              // float distanceToSurface = texture2D( sdfTex, uv ).r - 0.01;
              float distanceToSurface = texture2D( sdfTex, uv ).r - surface;
              if ( distanceToSurface < SURFACE_EPSILON ) {
                intersectsSurface = true;
                break;
            ...
            if ( intersectsSurface ) {
              ...
              if (renderstyle == 0)
                ...
                volumeColor = cast_iso(uv, step, nsteps, sdfRayDirection);
            }

            vec4 g = clamp(volumeColor - sceneColor * 0.5, 0.0, 1.0);

            if (length(volumeColor.xyz) < 0.01) { volumeColor = vec4(0.0); g = sceneColor; }
            if (length(sceneColor.xyz) < 0.01) { sceneColor = vec4(0.0); g = volumeColor; }
        ...
        vec4 cast_iso(vec3 start_loc, vec3 step, int nsteps, vec3 view_ray) {
          ...
          vec4 volumeColor = vec4(0.0);   // init transparent
            ...
            if (val > low_threshold) {
              ...
              for (int i=0; i<REFINEMENT_STEPS; i++) {
                ...
                if (val > renderthreshold) {
                  volumeColor = add_lighting(val, iloc, dstep, view_ray);
                  return volumeColor;
                }
                iloc += istep;
              }
